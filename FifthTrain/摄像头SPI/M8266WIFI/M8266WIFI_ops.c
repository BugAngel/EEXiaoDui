...
void M8266WIFI_Module_Hardware_Reset(void) // total 800ms
{
	 ...
	 M8266WIFI_Module_delay_ms(50); 	  // at least 18ms required for reset-out-boot sampling boottrap pin
...
#define SPI_BaudRatePrescaler_256       ((uint32_t)0x00000100U)
   ...
   M8266HostIf_SPI_SetSpeed(SPI_BaudRatePrescaler_8);					// Setup SPI Clock. Here 108/8 = 13.5MHz for STM32F7xx, up to 40MHz
	 spi_clk = 13500000;
#elif  defined(MCU_IS_NXP_LPC17XX)
   M8266HostIf_SPI_SetSpeed(SPI_BaudRatePrescaler_6);					// Setup SPI Clock. Here 96/6 = 16MHz for LPC17XX, upto 40MHz
	 spi_clk = 16000000;
...
#error YOU SHOULD DEFINED MCU_IS_XXX near line 17 in brd_cfg.h
   ...
   //It is very important to call M8266HostIf_SPI_Select() to tell the driver which SPI you used
	 ...
	 M8266HostIf_SPI_Select((uint32_t)M8266WIFI_INTERFACE_SPI, spi_clk);
...
#if 0 // test SPI communication is OK. You may give a loop for stress test. Comment it for formal release
{
	 ...
	 if(j>=10)
   {
	...
	if(M8266WIFI_SPI_Set_Opmode(1, 0, &status)==0)  // set to STA mode, not saved // 1=
    return 0;
#endif
  ...
  // get current op_mode, if STA mode or STA+AP mode, then, wait connection to external AP and getting ip
	...
	// alternative to AT or Airkiss/WPS, to update the default in spi flash
		 ...
		 //u8 M8266WIFI_SPI_STA_Connect_Ap(u8 ssid[13+1], u8 password[13+1], u8 saved, u8 timeout_in_s, u16* status);
		   if(M8266WIFI_SPI_STA_Connect_Ap("happy", "dianzisheji", 0, 20, &status) == 0) // not saved, timeout=20s
			 {
				 return 0;
			 }
...
u8 M8266WIFI_Config_Connection_via_SPI(u8 tcp_udp, u16 local_port, u8* remote_ip, u16 remote_port, u8 link_no)
{
  ...
  //  u8 M8266WIFI_SPI_Setup_Connection(u8 tcp_udp, u16 local_port, u8* remote_ip, u16 remote_port, u8 link_no, u8 timeout_in_s, u16* status);
	return M8266WIFI_SPI_Setup_Connection(tcp_udp, local_port, remote_ip, remote_port, link_no, 20, &status);
}
