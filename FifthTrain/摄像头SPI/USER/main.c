    ...
    if (ov2640_mode) //只有在JPEG格式下,才需要做处理.
    {
        ...
        if (jpeg_data_ok == 2) //上一次的jpeg数据已经被处理了
        {
            ...
            DMA_SetCurrDataCounter(DMA2_Stream1,
                                   jpeg_buf_size); //传输长度为jpeg_buf_size*4字节
...
void jpeg_test(void)
{
    ...
    u8 msgbuf[15];  //消息缓存区
    u8 jpeg_buf[TEST_SEND_DATA_SIZE];   //JPEG发送缓存
    u8  link_no = 0;
    u16 status = 0;
    ...
    DCMI_DMA_Init((u32)&jpeg_buf, jpeg_buf_size, DMA_MemoryDataSize_Word,
                  DMA_MemoryInc_Enable);//DCMI DMA配置
        ...
        if (jpeg_data_ok == 1) //已经采集完一帧图像了
        {
            ...
            for (i = 0; i < jpeg_data_len * 4; i++) //dma传输1次等于4字节,所以乘以4.
            {
                ...
                M8266WIFI_SPI_Send_Data(jpeg_buf, TEST_SEND_DATA_SIZE, link_no, &status);
								memset(jpeg_buf,0,sizeof(jpeg_buf));//清空数组
							}
                key = KEY_Scan(0);
                if (key)break;
            }
...
void rgb565_test(void)
{
    ...
    LCD_ShowString(30, 170, 200, 16, 16,
                   "KEY_UP:FullSize/Scale"); //1:1尺寸(显示真实尺寸)/全尺寸缩放
                ...
                case WKUP_PRES: //1:1尺寸(显示真实尺寸)/缩放
                    ...
                    if (scale == 0)
                    {
                        OV2640_ImageWin_Set((1600 - lcddev.width) / 2, (1200 - lcddev.height) / 2,
                                            lcddev.width, lcddev.height); //1:1真实尺寸
                        OV2640_OutSize_Set(lcddev.width, lcddev.height);
...
int main(void)
{
    ...
    while (1)
    {
        ...
        if (t == 100)LCD_ShowString(30, 150, 230, 16, 16,
                                        "KEY0:RGB565  KEY1:JPEG"); //闪烁显示提示信息
        if (t == 200)
        {
